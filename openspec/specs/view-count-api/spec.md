...
The endpoint SHALL increment `view_count` by 1 for the given news ID using a server-side atomic update via Postgres RPC (`increment_news_view_count`).

#### Scenario: Valid news ID
- **WHEN** POST /api/news/:id/view is called with a valid news UUID
- **THEN** `view_count` SHALL be incremented by 1 atomically and response SHALL be HTTP 200 with `{ data: null }`
