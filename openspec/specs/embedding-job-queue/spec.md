...
Async job queue for article embedding generation, following the same pattern as `view_count_jobs`.
...
- **WHEN** a failed job is re-enqueued (either by backfill or manual reset)
...
#### Scenario: Backfill with unprocessed articles
