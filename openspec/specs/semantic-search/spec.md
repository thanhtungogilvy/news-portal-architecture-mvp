...
Natural-language search over published articles using pgvector embeddings and LM Studio (local AI).

**Implemented in:** Phase 8.2 (`phase-8-semantic-search`)
...
### Requirement: Search results include normalized and raw similarity scores
...
#### Scenario: Score fields present on all results
- **WHEN** any search result is returned
- **THEN** each item SHALL have fields: `id`, `title`, `slug`, `summary`, `thumbnailUrl`, `category`, `score`, `rawScore`
...
- **THEN** URL SHALL update to `/search?q=<query>` so the search is shareable
...
When building embedding text, article content (HTML-stripped) SHALL be capped at 2000 characters to improve recall for queries referencing content deeper in the article.
