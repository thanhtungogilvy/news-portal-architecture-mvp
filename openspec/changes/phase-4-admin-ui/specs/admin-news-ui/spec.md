...
#### Scenario: Featured badge shown when applicable
- **WHEN** a news row has `is_featured = true`
- **THEN** the table SHALL show an additional badge indicating the article is featured
- **NOTE**: Deferred — `is_featured` column does not yet exist in the database schema. Implement after schema migration adds the column.
...
`AdminNewsForm` SHALL expose reactive form fields: title, slug, summary, body (textarea), category (select from categories), status (draft | published | archived), thumbnail_url, and published_at (optional date input). The `is_featured` checkbox is deferred pending a database schema migration to add the column.
