  ...
  <div class="bg-slate-50">
    ...
    <section class="flex flex-col gap-8 px-4 py-10 sm:px-6 sm:py-12 lg:flex-row lg:gap-12 lg:px-12 lg:py-16">
      ...
      >
        ...
        />
        <p
          class="absolute bottom-5 left-6 text-xs font-medium leading-[1.4] text-white/70"
        >
        ...
        <div v-if="hero" class="flex items-center gap-3">
          ...
          <div class="flex flex-col gap-0.5">
            <span class="text-base leading-[1.6] text-navy-900">{{
              hero.authorName ?? "Biên tập viên"
            }}</span>
            <span class="text-sm leading-[1.5] text-slate-500">
              Đọc {{ estimateReadTime(hero.content) }} phút ·
              {{ formatNewsDate(hero.publishedAt) }}
            </span>
          </div>
        </div>
        <NuxtLink
    ...
    <section class="flex flex-col gap-8 px-4 pb-10 sm:px-6 sm:pb-12 lg:px-12 lg:pb-16">
      ...
      <div class="flex items-center justify-between">
        <div class="flex flex-col gap-1">
          ...
          <h2
            class="font-vietnam font-semibold text-2xl leading-[1.25] text-navy-900"
          >
            Tin nổi bật vừa cập nhật
          </h2>
        </div>
        <NuxtLink
          to="/news"
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          <div class="flex flex-col gap-3">
            ...
            <h3
              class="font-vietnam font-semibold text-xl leading-[1.3] text-navy-900 group-hover:text-sage-600 transition-colors"
            >
              {{ item.title }}
            </h3>
            <p
              v-if="item.summary"
              class="line-clamp-2 text-base leading-[1.6] text-slate-600"
            >
              {{ item.summary }}
            </p>
            <div class="flex items-center gap-2 text-sm leading-[1.5]">
              ...
              <span class="text-slate-500"
                >Đọc {{ estimateReadTime(item.content) }} phút</span
              >
            </div>
          </div>
        </NuxtLink>
    ...
    <section class="bg-slate-100 px-4 py-10 flex flex-col gap-8 sm:px-6 sm:py-12 lg:px-12 lg:py-16">
      <div>
        ...
        <h2
          class="mt-1 font-vietnam font-semibold text-2xl leading-[1.25] text-navy-900"
        >
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div class="flex items-center justify-between pb-4">
            <h3
              class="font-vietnam font-semibold text-xl leading-[1.3] text-navy-900"
            >
              {{ col.name }}
            </h3>
          </div>
          ...
          >
            <span
              class="font-vietnam font-bold text-2xl leading-none text-slate-300 shrink-0"
            >
              {{ String(idx + 1).padStart(2, "0") }}
            </span>
            <NuxtLink
              :to="`/news/${article.slug}`"
              class="flex flex-1 flex-col gap-1.5 group"
            >
              <p
                class="font-vietnam font-medium text-base leading-[1.35] text-navy-900 group-hover:text-sage-600 transition-colors"
              >
                {{ article.title }}
              </p>
        ...
        <template v-if="categoryColumns.length === 0">
          <div
            v-for="i in 3"
            :key="i"
            class="flex flex-col rounded-lg border border-slate-100 bg-white p-6 animate-pulse gap-4"
          >
            <div class="h-6 w-32 rounded bg-slate-200" />
            <div
              v-for="j in 4"
              :key="j"
              class="flex gap-4 py-3 border-t border-slate-100 first:border-t-0"
            >
      ...
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
        ...
        >
          ...
          <div class="flex flex-col gap-4">
            <div
              class="flex items-center gap-3 text-xs font-medium leading-[1.4]"
            >
              <span class="tracking-[1px] text-sage-600 uppercase">{{
                item.category?.name ?? "Điều tra"
              }}</span>
              <span class="text-slate-500">·</span>
              <span class="tracking-[1px] text-slate-500 uppercase">{{
