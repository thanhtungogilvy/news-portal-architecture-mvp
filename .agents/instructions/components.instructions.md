...
- Components nằm dưới `app/components/` và được Nuxt auto-import.
- Repo đang cấu hình `components: [{ path: "~/components", pathPrefix: false }]`, nên tên component lấy theo filename, không lấy theo folder path.
- Vì `pathPrefix: false`, filename phải unique toàn project: không tạo cả `ui/Button.vue` và `forms/Button.vue`.
- Giữ prefix trong filename để tránh collision: `UiButton.vue`, `AppSidebar.vue`, `BuildingCard.vue`.
- Không import component thủ công từ path thật nếu auto-import hoặc `#components` dùng được.
- Component động dùng `resolveComponent("LiteralName")` hoặc import từ `#components`; không build tên component bằng string động.
- Link nội bộ dùng `<NuxtLink>` thay vì `<a>` hoặc `RouterLink`.
...
### Dùng lazy/client-only đúng chỗ

Component nặng hoặc không luôn hiển thị có thể lazy-load bằng prefix `Lazy`.

```vue
<!-- ✅ chỉ load chart khi panel mở -->
<LazyRevenueChart v-if="showChart" :series="series" />
```

Với component không cần interactive ngay, dùng lazy hydration:

```vue
<!-- ✅ hydrate khi vào viewport -->
<LazyActivityFeed hydrate-on-visible />
```

Browser-only component dùng suffix `.client.vue` hoặc `<ClientOnly>`.

```vue
<!-- app/components/charts/RevenueChart.client.vue -->
<ClientOnly>
  <RevenueChart />
  <template #fallback>
    <UiSkeleton />
...
Không dùng browser API trong setup SSR nếu chưa guard bằng `import.meta.client`, `onMounted`, `.client.vue`, hoặc `<ClientOnly>`.
